...
<Button onClick={async(e)=>{ e.preventDefault(); console.log(getValues());  if(! await trigger(['email','fullname','confirmpassword','password'])){  console.log(errors);return} await onGenerateotp(getValues('email'),getValues('password'),getValues('fullname'),setstep)}} className='w-full mt-5'>Continue</Button>
